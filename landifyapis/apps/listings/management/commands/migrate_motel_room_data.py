...
from apps.listings.models import (  # <<< Đảm bảo ListingCategory được import
    Listing,
    ListingCategory,
    ListingType,
    UnitPrice,
...
def find_best_ward_match(city_str, district_str, ward_str, preloaded_wards):
    if not city_str or not district_str or not ward_str:
        return None
    ...
    for ward_obj, normalized_names_new in preloaded_wards.items():
        if (
            normalized_names_new["ward"] == norm_ward_old
            and normalized_names_new["district"] == norm_district_old
            and normalized_names_new["city"] == norm_city_old
        ):
            return ward_obj
    potential_matches = []
    for ward_obj, normalized_names_new in preloaded_wards.items():
        if (
            norm_district_old in normalized_names_new["district"]
            and norm_city_old in normalized_names_new["city"]
            and norm_ward_old in normalized_names_new["ward"]
        ):
...
def parse_price(price_str, unit_str):
    if not price_str or not unit_str:
        return None, None
    try:
...
class Command(BaseCommand):
    ...
    def _generate_full_phone_number(self, masked_phone):
        if not masked_phone or not isinstance(masked_phone, str):
            return None
    ...
    def handle(self, *args, **options):
        ...
        try:
            preloaded_wards = {
                w: {
                    "city": normalize_string(w.parent_code.parent_code.name),
                    "district": normalize_string(w.parent_code.name),
                    "ward": normalize_string(w.name),
                }
                    ...
                    with transaction.atomic():
                        ...
                        user, user_created = User.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                "first_name": source_data.get("name_per", "Người dùng"),
                            },
                        )
                        if user_created:
                        ...
                        # Xử lý Location
                        ward_obj = find_best_ward_match(
                            source_data.get("city"),
                            source_data.get("district"),
                            source_data.get("ward"),
                            preloaded_wards,
                        )
                        if not ward_obj:
                            skipped_count += 1
                        ...
                        location_obj = Location.objects.create(
                            street=source_data.get("street", ""), ward=ward_obj, point=point_obj
                        )
                        ...
                        # Xử lý Listing
                        price_val, unit_price_obj = parse_price(
                            source_data.get("price"), source_data.get("unit_price")
                        )
