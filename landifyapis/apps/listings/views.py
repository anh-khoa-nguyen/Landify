...
from .models import Listing, ListingCategory, ListingType, Property, UnitPrice, UserPromotion, VipType
from .option_serializers import (
    DirectionOptionSerializer,
    LegalStatusOptionSerializer,
    ListingCategoryOptionSerializer,
    ListingTypeOptionSerializer,
    PropertyFeatureSerializer,
    PropertyTypeOptionSerializer,
    UnitPriceOptionSerializer,
    UserPromotionOptionSerializer,
...
class ListingViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['list', 'retrieve', 'search', 'potential', 'ai_summary']:
            return [permissions.AllowAny()]
        if self.action == 'create':
            return [perms.IsIdentityVerified()]
        if self.action in ['update', 'partial_update', 'destroy', 'update_features']:
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'list' or self.action == 'potential':
            return ListingPreviewSerializer
        if self.action == 'ai_summary':
            return ListingAISerializer
        if self.action == 'protest':
            return ProtestSerializer
        if self.action == 'wishlist':
            return WishlistSerializer

        # Mặc định cho 'retrieve', 'update_features', và các action khác trả về chi tiết
    ...
    def get_object(self):
        ...
        obj = get_object_from_public_id_or_404(queryset, public_id)
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            new_listing = listing_services.create_full_listing(user=request.user, validated_data=validated_data)
    ...
    def wishlist(self, request, public_id=None):
        ...
        try:
            toggle_status, wishlist_item = interactions_services.toggle_wishlist_item(user=user, listing=listing)

            if toggle_status == "added":
                serializer = self.get_serializer(wishlist_item)
                return Response({
                    "status": "added", "message": "Đã thêm vào danh sách yêu thích.",
                    "wishlist_item": serializer.data
                }, status=status.HTTP_201_CREATED)
            else:  # toggle_status == "removed"
                return Response({
                    "status": "removed", "message": "Đã xóa khỏi danh sách yêu thích."
    ...
    def potential(self, request):
        ...
        potential_listings_qs = listing_services.find_potential_listings(latitude=lat, longitude=lng, filters=filters)
        ...
        serializer = self.get_serializer(potential_listings_qs, many=True)
    ...
    @action(methods=["post"], detail=True, url_path="protest")
    def protest(self, request, public_id=None):
        """
        Cho phép người dùng gửi kháng nghị cho tin đăng của họ.
        Endpoint này chỉ là một lối tắt tiện lợi.
        """
        listing = self.get_object()
        # Logic kiểm tra tin đăng có phù hợp để kháng nghị không (ví dụ: đang bị ẩn)
        if listing.active is True:
            return Response({"error": "Không thể kháng nghị một tin đăng đang hoạt động."},
                            status=status.HTTP_400_BAD_REQUEST)

        # get_serializer() sẽ tự động trả về ProtestSerializer
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        # TODO: Triển khai logic tạo Protest trong services
        # protest = moderation_services.create_protest_for_listing(
        #     listing=listing,
        #     protester=request.user,
        #     reason=serializer.validated_data.get('reason')
        # )

        # serializer.instance = protest # Gán instance mới tạo vào serializer
...
class ListingOptionsView(APIView):
    ...
    def get(self, request, format=None):
        ...
        valid_promotions = UserPromotion.objects.filter(
            user=request.user, status=UserPromotion.Status.AVAILABLE, expiry_date__gte=timezone.now()
        )
        categories = (
            ListingCategory.objects.select_related("listing_type", "property_type")
            .prefetch_related("applicable_features", "listing_type__applicable_unit_prices")  # Prefetch cả unit_prices
            .filter(active=True)
        )
        ...
        for category in categories:
            ...
            grouped_categories[lt.code]["categories"].append(ListingCategoryOptionSerializer(category).data)
