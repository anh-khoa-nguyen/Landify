...
from .models import ModerationAction, Protest, Report, User
...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        serializer.save(reporter=self.request.user, reported_item_type=serializer.validated_data["reported_item_type"])
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            moderation_services.create_protest(
                action=action, protester=self.request.user, reason=serializer.validated_data.get("reason")
            )
