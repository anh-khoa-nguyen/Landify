...
class PropertyAdmin(admin.ModelAdmin):  # <-- THAY ĐỔI: Quay lại ModelAdmin thông thường
    list_display = (
        "id", "owner_link", "property_type", "display_location",
        "area", "media_count", "listing_count", "active"
    )
    list_filter = (
        "active", "property_type", "legal_status",
        "location__ward__parent_code__parent_code",
    )
    search_fields = (
        "owner__username", "location__street", "location__ward__name",
