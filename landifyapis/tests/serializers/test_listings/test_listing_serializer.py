...
from landifys.models import PropertyFeature, ListingPropertyFeatureValue # <-- Thêm import
...
class TestListingSerializer:
    ...
    def test_serialization_contains_all_nested_objects(self, listing_factory):
        ...
        ListingPropertyFeatureValue.objects.create(
            listing=listing,
            feature=feature,
            value=3.0
        )
        ...
        expected_keys = {
            "public_id", "user", "property", "listing_type", "listing_type_name",
            "title", "content", "price_value", "unit_price", "unit_price_name",
            "display_price", "status", "spam_check_status", "commission_percentage",
            "created_date", "feature_values", "buysell_detail", "rental_detail",
            "project_detail"
        }
