...
def test_change_password_with_incorrect_old_password(log_step):
    ...
    with pytest.raises(BusinessLogicError, match="Mật khẩu cũ không chính xác."):
        accounts.change_user_password(user=user, old_password="wrong_old_password", new_password="new_password")
    log_step("=> PASSED!")
...
def test_change_password_successfully(log_step):
    ...
    accounts.change_user_password(user=user, old_password="old_password", new_password="new_password_123")
