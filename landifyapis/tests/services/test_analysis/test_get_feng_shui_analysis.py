...
from landifys.services import analysis, BusinessLogicError
...
def test_get_feng_shui_analysis_successfully(mock_fengshui, log_step):
    ...
    mock_fengshui.analyze_feng_shui_rules.return_value = {
        "score": 95,
    ...
    result = analysis.get_feng_shui_analysis(
        property_direction_name=direction,
        date_of_birth=dob
    )
    ...
    mock_fengshui.analyze_feng_shui_rules.assert_called_once_with(
        user_menh="Kim",
...
def test_get_feng_shui_analysis_fails_if_menh_not_found(mock_fengshui, log_step):
    ...
    with pytest.raises(BusinessLogicError, match="Không thể xác định Mệnh cho năm sinh 1990."):
        analysis.get_feng_shui_analysis(
            property_direction_name="Đông",
